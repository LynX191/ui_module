...
namespace rviz
{
	VisualizationFrame::VisualizationFrame(QWidget* parent)
		: QMainWindow(parent), app_(nullptr), render_panel_(nullptr), show_help_action_(nullptr), preferences_(new Preferences()),
		  file_menu_(nullptr), recent_configs_menu_(nullptr), toolbar_(nullptr), manager_(nullptr), splash_(nullptr),
		  toolbar_actions_(nullptr), show_choose_new_master_option_(false), add_tool_action_(nullptr), remove_tool_menu_(nullptr),
		  initialized_(false), geom_change_detector_(new WidgetGeometryChangeDetector(this)), loading_(false),
		  post_load_timer_(new QTimer(this)), frame_count_(0), toolbar_visible_(true)
	{
		// Some init Iric implement
		is_close_done = false;
		// End
		...
		package_path_ = ros::package::getPath("tomo_rviz");
		...
		// display_button_ptr = new QToolButton();
		// display_button_ptr->setText("Display Panel");
		// display_button_ptr->setContentsMargins(0, 0, 0, 0);
		// statusBar()->addPermanentWidget(display_button_ptr, 0);
		// connect(display_button_ptr, SIGNAL(clicked(bool)), this, SLOT(displayPanel()));
		// status_label_ = new QLabel("");
		// statusBar()->addPermanentWidget(status_label_, 1);
		// motion_button_ptr = new QToolButton();
		// motion_button_ptr->setText("Motion Panel");
		// motion_button_ptr->setContentsMargins(0, 0, 0, 0);
		// statusBar()->addPermanentWidget(motion_button_ptr, 0);
		// connect(motion_button_ptr, SIGNAL(clicked(bool)), this, SLOT(motionPanel()));
	...
	{
		delete display_button_ptr;
		delete motion_button_ptr;
	...
	void VisualizationFrame::initialize(const QString& display_config_file)
	{
		...
		if(!ros::isInitialized()) {
			int argc = 0;
			ros::init(argc, nullptr, "visualizer_app", ros::init_options::AnonymousName);
		}
	...
	void VisualizationFrame::loadPersistentSettings()
	{
		...
		if(!reader.error()) {
			...
			if(config.mapGetString("Last Config Dir", &last_config_dir) && config.mapGetString("Last Image Dir", &last_image_dir)) {
	...
	void VisualizationFrame::initMenus()
	{
		...
		//     addAction(file_menu_quit_action);
	...
	void VisualizationFrame::openNewPanelDialog()
	{
		...
		//    NewObjectDialog* dialog = new NewObjectDialog(panel_factory_, "Panel", empty, empty, &class_id, &display_name,
		//    this);
		//	manager_->stopUpdate();
		//    if(dialog->exec() == QDialog::Accepted) {
	...
	void VisualizationFrame::loadDisplayConfig(const QString& qpath)
	{
		...
		// if(!valid_load_path) {
		// 	actual_load_path = (fs::path(package_path_) / "moveit.rviz");  // default demo moveit
		// 	// actual_load_path = "/home/omen/ws_tomo/src/configuration/tomoseq_config/launch/moveit.rviz";
		// 	if(!(valid_load_path = fs::is_regular_file(actual_load_path))) {
		...
		loadDisplayConfigHelper(getenv("HOME") + std::string("/tomo_config/moveit_ui.rviz"));
	}
	...
	{
		//	manager_->stopUpdate();
		//	QString filename = QFileDialog::getOpenFileName(this, "Choose a file to open", QString::fromStdString(last_config_dir_),
	...
	{
		manager_->stopUpdate();
		QString q_filename = QFileDialog::getSaveFileName(this, "Choose a file to save to", QString::fromStdString(last_config_dir_),
														  "RViz config files (" CONFIG_EXTENSION_WILDCARD ")");
		manager_->startUpdate();
	...
	void VisualizationFrame::showPlugin(bool show)
	{
		if(display_panel_pdw_ptr && motion_planing_pdw_ptr) {
			display_panel_pdw_ptr.get()->showPlugin(show);
			motion_planing_pdw_ptr.get()->showPlugin(show);
		}
	}

	void VisualizationFrame::toolPluginChanged(bool& state)
	{
		// display_button_ptr->setVisible(state);
		// motion_button_ptr->setVisible(state);
		// statusBar()->setVisible(state);
	}

	QDockWidget* VisualizationFrame::addPanelByName(const QString& name, const QString& class_id, Qt::DockWidgetArea area, bool floating)
	{
		QString error;
		...
		if(name == "Displays") {
			display_panel_pdw_ptr.reset(record.dock);
		}
	...
	PanelDockWidget* VisualizationFrame::addPane(const QString& name, QWidget* panel, Qt::DockWidgetArea area, bool floating)
	{
		...
		if(name == "MotionPlanning") {
			motion_planing_pdw_ptr.reset(dock);
		}
